...
const newEdges = [
  {
    ...
    source: '039b794b-b8b7-47b3-9914-77df8d0e30ee',  // input_node
...
// ── Rebuild output in original string format ────────────────────────────────
const output = {
  ...input,
...
delete output.checksum; // platform regenerates on import

fs.writeFileSync(outputPath, JSON.stringify(output, null, 2));

console.log(`Done — saved to ${outputPath}`);
console.log(`  Nodes: ${nodes.length} before  ->  ${filteredNodes.length} after  (-4 old init, +2 consolidated)`);
console.log(`  Edges: ${edges.length} before  ->  ${filteredEdges.length} after  (-6 old, +4 new)`);
